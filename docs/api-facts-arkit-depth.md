...
## 六之二、真机实测结果（2026-08-11，iPad Pro 11-inch M4 / iPad16,4）

> 以下为探针在真机上的实测值，**已由未知量转为已知量**。测量方式与判据见探针源码与本节说明。

| 编号 | 实测值 | 结论 |
|---|---|---|
| P1 | `1440x1080@60fps`、`1440x1080@30fps`、`1280x720@60fps`、`1280x720@30fps` | 人脸追踪在 M4 iPad 上可用，**有 60fps 档**，首项为默认 |
| P2 | 静止时 `cam pos (0,0,0)` | 仅报数未下结论；世界原点位置本项目不依赖（走 `viewMatrix(for:)`） |
| P3 | **60.0 fps**，帧间隔 16.7–16.7 ms（120 样本） | 抖动极小，滤波器参数可按 60Hz 定 |
| P4 | **3** | 可同时追踪 3 张脸（`maximumNumberOfTrackedFaces` 默认仍为 1） |
| P5 | 见下方专节 | **左右眼命名是镜像的** |
| P6 | `lookAtPoint` 模长约 0.92–0.93 | 尺度约 1 米量级，头文件未定义其含义，本项目不使用 |
| P7 | 1220 顶点，x∈[-0.072,0.072] y∈[-0.088,0.090] z∈[-0.025,0.071] | 确认为以头部为中心的局部空间，单位米 |
| P8 | **nil** | ✅ **实测印证 §2.8 的推断**：前摄被 ARKit 独占，不能并存 `AVCaptureSession` |
| P9 | **0.0615 m**（120 样本均值） | ✅ **眼位单位确认为米**。落在成人瞳距 50–75mm 区间 |
| P10 | **`fdep` 640×480，`isDepthDataFiltered = false`** | 是 **depth（米）不是 disparity**，走 `.depth` 分支；**未滤波意味着缺失像素为 NaN** |
| P11 | `iPad16,4`；点 834×1210 @2.0x；像素 1668×2420 | 见 `DeviceProfile` 表 |
| P12 | **(-0.0126, -0.1024, -0.4526) m**，模长约 0.45 | ✅ **相机空间 Z 为负** → 换算到屏幕空间**必须翻转 Z** |
| P13 | 界面方向 `portrait`（因 app 锁定竖持） | 探针报的是**界面**方向而非物理方向，见下方「已知缺陷」 |

### P5：左右眼命名是镜像的（两条独立证据）

**证据一 · 转头**：请测试者向右转头（鼻子朝其右手边），此时其**左脸转向镜头、左眼物理上更靠近镜头**。实测：

```
L.z = -0.3873    R.z = -0.3383    →  R 离镜头近 4.9cm
```

更近的那只被标为 `rightEyeTransform` → **`rightEyeTransform` 指的是测试者自己的左眼**。

**证据二 · 眨眼**：测试者闭**左**眼，激活的是 `eyeBlinkRight`（0.84 vs 0.20）。

两条独立物理动作结论一致：**ARKit 的 left/right 按捕获图像命名（镜像），不是按解剖学左右。投影时必须换向。**

这也解释了 `$AR/ARFaceAnchor.h:14-21` 那段绕口的 blendShape 镜像说明——`eyeTransform` 遵循同一套命名约定。

### P9 与 P5 互相印证单位

两个独立测量：瞳距均值 **0.0615 m**，两眼连线向量模长 **0.0614 m**，差 0.1mm。眼位单位是米这件事有两条独立证据。

### 探针的已知缺陷（供后续修正）

- **P13 报的是界面方向不是物理设备方向**。本次因 app 锁定 `portrait` 而能反推出物理横持，但换个场景会误导。应改用 `UIDevice.current.orientation`。
- **P11 只能报机型标识与推算尺寸**，摄像头物理偏移仍需外部数据源，不能靠尺量（用户不可能拥有所有机型）。

---

## 六之三、设备物理参数的权威数据源（2026-08-11 发现）

**Apple 官方逐机型工程图纸，免登录免费下载，标注精度 0.01mm：**

> <https://developer.apple.com/accessories/dimensional-drawings/>
> 直链规律：`https://developer.apple.com/download/files/accessories/dimensional-drawings/<slug>.pdf`
> 例：`ipad-pro-11-inch-m4.pdf`、`iphone-14-pro-max.pdf`

这是给配件厂商的机械图纸。**规格页、用户手册、开发者文档里都没有的摄像头坐标，在这里有。**
覆盖 iPhone 12 起、iPad 8 代起；iPhone X/XS/XR/11 与更老 iPad 无图纸。

### 三条已验证的关键性质

1. **显示区严格居中于产品外形。** 逐机型验证：
   - iPhone 14 Pro Max：77.58 − 2×3.19 = 71.20 ≈ 图标 71.21 ✅
   - iPad Pro 11 M4：(177.51 − 160.13)/2 = 8.69 四边相等 ✅

   因此 `摄像头相对屏幕中心的偏移 = 摄像头距产品边距离 − 产品尺寸/2`，不必单独求显示区位置。

2. **图纸尺寸与 ppi 反算值有 0.5mm 级差异，以图纸为准。**
   iPad Pro 11 M4 显示区图纸 232.32×160.13，按 2420×1668 @264ppi 算是 232.83×160.48。ppi 是取整值。

3. **iPad 给单个器件光轴坐标，刘海/灵动岛 iPhone 只给合并包络。** 这是唯一的短板：
   - iPad Pro 11 M4 的 DETAIL F 逐个标注 FCAM / IR CAM / DOT PROJECTOR / FLOOD ILLUMINATOR / FRONT ALS / FRONT MIC，配纵坐标链 103.65 / 110.90 / 116.20 / **124.85** / 133.50 / 140.30，其中 124.85 = 249.70/2 **正落在 FCAM 上**（与 Apple 手册「front camera at the center right」互证）
   - iPhone 14 Pro Max 只有「FRONT CAMERA KEEPOUT」包络宽 20.76mm，**摄像头在包络内的横向位置不给**

### 读图注意事项

- **图纸用第三角投影**，务必确认所读视图是正视（用户看屏幕方向）还是背视。**X 符号搞反会让误差翻倍。**
- pdftotext 提取的数字**可能出错**：本次 116.20 被读成 118.20、133.50 被读成 139.50。**必须渲染成图亲眼核对。**
- 形如 `121.01° FCAM KEEPOUT` 的是**视场角**不是坐标，别混。

### 走不通的路（已实测证伪，不要重复浪费时间）

| 路径 | 结论 |
|---|---|
| Apple Design Resources 的设备模板 | 只有 PNG / Photoshop 位图，无任何毫米标注 |
| `ARCamera.intrinsics` | 内参描述「相机→图像」映射，主点偏移是模组内部属性。**数学上不含摄像头在机壳中位置的自由度** |
| `AVCameraCalibrationData.extrinsicMatrix` | 官方原文「expressed with respect to a **reference camera**」——是相机对相机，不涉及屏幕 |
| iFixit 拆解 | 只有维修步骤图文，无尺寸标注 |
| GazeCapture 数据集 | 表结构完全同构且明示源自 Apple 图纸，但只到 iPhone 6s，无一台支持 ARKit 人脸追踪。**可用于验证读图方法**（其 iPhone 6s 的 22.92/9.08 与 SE3 图纸的 22.99/9.10 吻合 0.07mm） |

### 精度需求（几何推导）

屏幕位置误差 `Δs = δ · b/(d+b)`，b→∞ 时趋近 δ；角误差 ≈ δ/d。

| 偏移误差 δ | d=350mm（手机） | 判定 |
|---|---|---|
| ≤2mm | 0.33° | 不可察觉，**目标值** |
| 5mm | 0.82° | 正常使用无感 |
| 10mm | 1.64° | 开始读作「有点偏」 |
| >20mm | >3° | 明显错误 |
| ~64mm（完全不修正） | 10.4° | 幻觉彻底崩塌 |

**两条重要推论：**
- **恒定偏移不破坏视差增益。** δ 是常量，`ds/dE` 不含 δ——头动时画面响应速率完全正确，δ 只造成静态视点平移。所以判据不是「视差对不对」而是「用户凭什么察觉这个静态偏移」。
- **δz 可忽略。** 取 |P−E|=200mm、b=200、d=350，δz=2mm 只造成 0.26mm 屏上偏移。三轴里只有 X、Y 需认真对待。
- **ARKit 自身的眼位就有 3~10mm 误差**（`ARFaceAnchor` 原点在鼻梁后方，眼球关节非节点），所以把摄像头偏移压到 1mm 以下无实际收益。
